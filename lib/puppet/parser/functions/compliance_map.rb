module Puppet::Parser::Functions
    ...
    def self.get_compliance_profiles
    ...
    def self.get_reference_map
      ...
      if ( !reference_map || reference_map.empty? )
        ...
        if ( !reference_map || reference_map.empty? )
          compliance_resource = catalog.resource('Class[compliance_markup]')
    ...
    def self.validate_reference_map(reference_map)
    ...
    def self.custom_call_file_info
      file_info = {
        :file => @source.file,
    ...
    def self.add_file_to_client(config, compliance_map)
    ...
    def self.write_server_report(config, compliance_map)
      report_dir = File.join(config[:server_report_dir], lookupvar('fqdn'))
      ...
      if config[:catalog_to_compliance_map]
        File.open(File.join(report_dir,'catalog_compliance_map.yaml'),'w') do |fh|
          fh.puts(compliance_map.catalog_to_map(@resource.scope.catalog))
        end
      end
    end
    ...
    if hitchhiker
      ...
      compliance_map = PuppetX.const_get("SIMP#{Puppet[:environment]}").const_get('ComplianceMap').new(compliance_profiles, main_config)
    end
    ...
    if main_config[:custom_call]
      ...
      compliance_map.process_catalog(@resource.scope.catalog, reference_map)
